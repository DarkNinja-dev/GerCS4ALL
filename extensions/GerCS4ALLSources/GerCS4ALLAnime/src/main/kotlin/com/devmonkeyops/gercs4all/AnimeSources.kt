...
@CloudstreamPlugin
class GerCS4ALLAnimePlugin : BasePlugin() {
    override fun load() {
...
/** WordPress's public search API gives a stable, pagination-independent search surface. */
private data class WordPressSearchItem(
    @JsonProperty("title") val title: String,
    @JsonProperty("url") val url: String,
)

abstract class WordPressAnimeProvider(
    ...
    override var lang = siteLanguage
    override val supportedTypes = setOf(TvType.Anime, TvType.AnimeMovie, TvType.OVA)

    override suspend fun search(query: String): List<SearchResponse> {
        val encoded = URLEncoder.encode(query, "UTF-8")
        val results = app.get("$mainUrl/wp-json/wp/v2/search?search=$encoded&per_page=30")
            .parsed<ArrayList<WordPressSearchItem>>()
        return results.map { result ->
            newAnimeSearchResponse(result.title.stripHtml(), result.url, TvType.Anime)
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val poster = document.selectFirst(
            "meta[property=og:image], .post-thumbnail img, .entry-content img, article img"
        ...
        links.forEach { link ->
            when {
                link.isGoogleDriveFile() -> callback(
                    newExtractorLink(name, "$name – Google Drive", directGoogleDrive(link)) {
                        referer = mainUrl
                    }
                )
    ...
    /** Resolves normal Google Drive *file* URLs. Folders deliberately are not
     * exposed as streams: they cannot be played by CloudStream without a file ID. */
    private suspend fun directGoogleDrive(url: String): String {
        val id = Regex("(?:/d/|[?&]id=)([^/?&]+)").find(url)?.groupValues?.getOrNull(1)
            ?: return url
...
class KayoAnime : WordPressAnimeProvider("KayoAnime", "https://kayoanime.com", "en")

class AnimeToast : WordPressAnimeProvider("AnimeToast", "https://www.animetoast.cc", "de")
